...
export type Mode = 'cafe' | 'restaurant' | 'daily';
...
export function newRoom(input: Partial<Room>): Room {
  ...
  return { id: input.id || String(Math.floor(10000 + Math.random() * 90000)), name: String(input.name || '未命名聚餐').slice(0, 120), mode: input.mode === 'restaurant' || input.mode === 'daily' ? input.mode : 'cafe', creator: nickname, creatorKey: input.creatorKey ? String(input.creatorKey) : undefined, duitNowId: String(input.duitNowId || ''), bankAccount: '', paymentQrUrl: '', splitMode: 'items', splitParts: 1, isClosed: false, members: [{ nickname, joinedAt: Date.now() }], orders: [], createdAt: Date.now() };
}
