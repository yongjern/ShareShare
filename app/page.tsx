...
export default function HomePage() {
  ...
  return <main className={`min-h-screen ${darkMode ? 'dark bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
    ...
    </div><div className="mt-12 grid gap-4 md:grid-cols-2"><ModeCard icon={<Coffee size={22}/>} title={t.cafe} description={t.cafeDesc} tags={t.cafeTags} active={mode === 'cafe'} onClick={() => setMode('cafe')} action={t.create}/><ModeCard icon={<Utensils size={22}/>} title={t.restaurant} description={t.restaurantDesc} tags={t.restaurantTags} active={mode === 'restaurant'} onClick={() => setMode('restaurant')} action={t.create}/></div>
      <div className="mt-10 grid gap-8 border-t border-slate-200 pt-8 sm:grid-cols-4">{t.steps.map((step, i) => <div key={step} className="flex gap-3"><span className="text-sm font-bold text-orange-600">0{i + 1}</span><span className="text-sm font-semibold text-slate-700">{step}</span></div>)}</div>
    </section>
